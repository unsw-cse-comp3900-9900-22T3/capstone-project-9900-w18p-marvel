import { AAvatar } from "./AAvatar"

interface ProfileCardProps {}

const ProfileCard = ({}: ProfileCardProps) => {
    const url = 'data:image/jpeg;base64,'
    return (
        <div className="flex bg-gray-700">
            <div className="flex flex-col items-stretch bg-white m-10 w-[730px] h-[300px] rounded-lg">
                <div className="divide-y divide-gray-300">
                    <div className="flex flex-basis: | auto ml-8 bg-white w-[690px] h-[100px]" >
                        <div className="mt-2">
                        <AAvatar src={url} size="lg" rounded="sm" />
                        </div>
                        <div className="text-sm font-bold text-gray-500 m-2">USER NAME</div>
                        <div className="text-sm font-bold text-gray-500 m-2">POSITION</div>
                
                    </div>
                    <div className="flex flex-basis: | auto ml-8 bg-white w-[690px] h-[100px]" >
                        <div className="flex1">
                            <div className="text-sm font-bold text-gray-500 m-2">EMAIL ADDRESS</div>
                            <div className="text-sm font-bold text-black m-2">1234@gmail.com</div>
                            
                        </div>
                        <div className="flex1">
                            <div className="text-sm font-bold text-gray-500 m-2">PHONE NUMBER</div>
                            <div className="text-sm font-bold text-black m-2">04502222</div>
                         </div>
                         <div className="flex1">
                         <div className="text-sm font-bold text-gray-500 m-2">USER ID</div>
                         <div className="text-sm font-bold text-black m-2">00001</div>
                         </div>
                    </div>

                    <div className="flex flex-basis: | auto ml-8 bg-white w-[690px] h-[100px]">
                        <div className="text-sm font-bold text-gray-500 m-2">edit profile</div>
                        <div className="text-sm font-bold text-black m-2">save profile</div>
                    </div>
                </div>
            </div>
        </div>
  )
  
  
  
//   <div className="text-yellow-500">Write your component here</div>;
};

export { ProfileCard };
